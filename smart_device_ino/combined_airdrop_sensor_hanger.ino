...
// Pin definitions from both files
...
// WiFi configuration (using hanger.ino settings)
const char *ssid = "OPPO Reno12 5G";
const char *password = "z63ri8fz";

// MQTT configuration (combining both)
...
// MQTT broker parameters (using hanger.ino settings)
...
// Variables from water_sensor.ino
bool lastRainState = false;

// Variables from hanger.ino
...
void loop() {
    ...
    // Rain sensor logic from water_sensor.ino
    ...
    if (currentRainState != lastRainState) {
        ...
        if (currentRainState) {
            ...
            client.publish(pub_rain_topic, "True");
        } else {
            Serial.println("No rain.");
            client.publish(pub_rain_topic, "False");
        }
    }
